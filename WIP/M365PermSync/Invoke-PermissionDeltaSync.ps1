...
# ══════════════════════════════════════════════════════════════
# Shared: Replace-FolderPermissions
# Used by Phase 0 (mini-baseline) and Phase 6 (re-fetch).
# Re-fetches all folder permissions for one mailbox from
# Graph (folder list) + Admin API (permissions per folder),
# then atomically replaces in DB (soft-delete + insert).
#
# Fix E: Using the same Admin API source as the baseline ensures
# AccessRights format consistency — no mismatch between baseline
# role names and delta cmdlet parameter values.
# ══════════════════════════════════════════════════════════════

function Replace-FolderPermissions {
    param(
        [System.Data.SqlClient.SqlConnection]$Conn,
        [guid]$ExchangeGuid,
        [string]$Upn
    ...
    # Collect all folder permissions for this mailbox first
    ...
    # Atomic replace: soft-delete existing + insert current
    ...
    try {
        foreach ($mailbox in $newMailboxes) {
            ...
            try {
                ...
                # Folder permissions — re-fetch via Graph + Admin API
                Replace-FolderPermissions -Conn $conn -ExchangeGuid $guid -Upn $upn
...
UPDATE dbo.SyncLog SET CompletedAt=SYSUTCDATETIME(), Status='Success',
    NewMailboxesBaselined=@New, ErrorCount=@Err, TokenAdvancedTo=@Token WHERE RunId=@RunId
"@ -Parameters @{
...
try {
    ...
    # Fix F: Defensive RawEventData parsing — the hunting API sometimes
    # returns dynamic columns as escaped JSON strings instead of nested objects.
    foreach ($r in $huntingResults) {
        if ($r.RawEventData -is [string]) {
            try   { $r.RawEventData = $r.RawEventData | ConvertFrom-Json }
            catch { Write-LogWarning "Failed to parse RawEventData for event at $($r.Timestamp): $($_.Exception.Message)" }
        }
    }
}
catch {
...
# ══════════════════════════════════════════════════════════════
# PHASE 6 — Folder permissions (event-driven re-fetch)
#
# Fix E: Instead of parsing AccessRights from audit events (which
# may differ in format from the Admin API — role names vs expanded
# rights), we identify WHICH mailboxes had folder changes and then
# re-fetch all folder permissions for those mailboxes from the
# same Admin API source the baseline uses. This ensures format
# consistency and also fixes issue J (folder names containing ':\').
#
# Uses Replace-FolderPermissions — same function as Phase 0.
# ══════════════════════════════════════════════════════════════
...
if ($folderEvents.Count -eq 0) {
    ...
    Write-LogInfo "$($folderEvents.Count) folder permission event(s) — collecting affected mailboxes"

    # Extract unique affected mailboxes from folder events
    $affectedFolderGuids = [System.Collections.Generic.HashSet[string]]::new(
        [System.StringComparer]::OrdinalIgnoreCase
    )
    $guidToMb = @{}
    ...
    foreach ($e in $folderEvents) {
        ...
        # Extract mailbox part from folder identity (mailbox@domain.com:\Calendar)
        # or fall back to ObjectName for events that don't use folder identity format
        $mailboxPart = $identity
        $colonIdx    = $identity.IndexOf(':\')
        if ($colonIdx -ge 0) { $mailboxPart = $identity.Substring(0, $colonIdx) }

        $mb = Resolve-MailboxGuid $mailboxPart
        if ($mb) {
            [void]$affectedFolderGuids.Add($mb.ExchangeGuid)
            $guidToMb[$mb.ExchangeGuid.ToString().ToLower()] = $mb
        }
        else {
            $counters.Skipped++
        }
    ...
    Write-LogInfo "Re-fetching folder permissions for $($affectedFolderGuids.Count) affected mailbox(es)"
    $phaseSw = [System.Diagnostics.Stopwatch]::StartNew()

    $conn = Open-SqlConnection
    try {
        foreach ($guidStr in $affectedFolderGuids) {
            $mb = $guidToMb[$guidStr.ToLower()]
            if (-not $mb) { continue }

            try {
                Write-LogInfo "  Folder re-fetch: $($mb.UserPrincipalName)"
                Replace-FolderPermissions -Conn $conn -ExchangeGuid ([guid]$mb.ExchangeGuid) -Upn $mb.UserPrincipalName
            }
            catch {
                $counters.Errors++
                Write-LogWarning "  Folder re-fetch failed for $($mb.UserPrincipalName): $($_.Exception.Message)"
            }
        }
    }
    finally {
...
UPDATE dbo.SyncLog SET
    ...
    Status                     = @Status,
    NewMailboxesBaselined      = @NewMailboxes,
